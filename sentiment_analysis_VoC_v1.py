...
if filename is not None:
    ...
    pos = pos.sort_values(['score'], ascending=False)
    st.write("1. " + str(pos['brand'][0]) + " | Positive | Sentiment Score: " + str(pos['score'][0]) + " - " + str(pos['body'][0]))
    st.write("2. " + str(pos['brand'][1]) + " | Positive | Sentiment Score: " + str(pos['score'][1]) + " - " + str(pos['body'][1]))
    st.write("3. " + str(pos['brand'][2]) + " | Positive | Sentiment Score: " + str(pos['score'][2]) + " - " + str(pos['body'][2]))
    st.write("4. " + str(pos['brand'][3]) + " | Positive | Sentiment Score: " + str(pos['score'][3]) + " - " + str(pos['body'][3]))
    st.write("5. " + str(pos['brand'][4]) + " | Positive | Sentiment Score: " + str(pos['score'][4]) + " - " + str(pos['body'][4]))

    st.markdown("------------------------------------------------------------------------------------")
    st.subheader("Top 5 negative reviews for Huawei :")
    neg = data[(data['brand'] == 'Huawei') & (data['score'] < .1)].reset_index()
    neg = neg.sort_values(['score'], ascending=True)
    st.markdown("1. " + str(neg['brand'][0]) + " | Negative | Sentiment Score: " + str(neg['score'][0]) + " - " + str(neg['body'][0]))
    st.markdown("2. " + str(neg['brand'][1]) + " | Negative | Sentiment Score: " + str(neg['score'][1]) + " - " + str(neg['body'][1]))
    st.markdown("3. " + str(neg['brand'][2]) + " | Negative | Sentiment Score: " + str(neg['score'][2]) + " - " + str(neg['body'][2]))
    st.markdown("4. " + str(neg['brand'][3]) + " | Negative | Sentiment Score: " + str(neg['score'][3]) + " - " + str(neg['body'][3]))
    st.markdown("5. " + str(neg['brand'][4]) + " | Negative | Sentiment Score: " + str(neg['score'][4]) + " - " + str(neg['body'][4]))

    # SAMSUNG
    st.markdown("------------------------------------------------------------------------------------")
    st.subheader("Top 5 positive reviews for Samsung :")
    pos = data[(data['brand'] == 'Samsung') & (data['score'] > .9)].reset_index()
    pos = pos.sort_values(['score'], ascending=False)
    st.write("1. " + str(pos['brand'][0]) + " | Positive | Sentiment Score: " + str(pos['score'][0]) + " - " + str(pos['body'][0]))
    st.write("2. " + str(pos['brand'][1]) + " | Positive | Sentiment Score: " + str(pos['score'][1]) + " - " + str(pos['body'][1]))
    st.write("3. " + str(pos['brand'][2]) + " | Positive | Sentiment Score: " + str(pos['score'][2]) + " - " + str(pos['body'][2]))
    ...
    st.markdown("------------------------------------------------------------------------------------")
    st.subheader("Top 5 negative reviews for Samsung :")
    neg = data[(data['brand'] == 'Samsung') & (data['score'] < .1)].reset_index()
    neg = neg.sort_values(['score'], ascending=True)
    st.markdown("1. " + str(neg['brand'][0]) + " | Negative | Sentiment Score: " + str(neg['score'][0]) + " - " + str(neg['body'][0]))
    st.markdown("2. " + str(neg['brand'][1]) + " | Negative | Sentiment Score: " + str(neg['score'][1]) + " - " + str(neg['body'][1]))
    st.markdown("3. " + str(neg['brand'][2]) + " | Negative | Sentiment Score: " + str(neg['score'][2]) + " - " + str(neg['body'][2]))
